/**
 * 哈希工具。rawHash/semanticHash/contentHash 均为 SHA-256 的 hex 小写。
 * 生产路径优先使用 Web Crypto (crypto.subtle)；同步兜底实现为 FIPS 180-4
 * 纯 JS 版本（DocumentCodec 需要同步哈希以保持 Interface 同步）。
 */
...
/**
 * 严格 UTF-8 解码；拒绝 BOM 与替换字符（文档协议第 6.2 节）。
 * 返回 null 表示非法编码或含 BOM。
 */
